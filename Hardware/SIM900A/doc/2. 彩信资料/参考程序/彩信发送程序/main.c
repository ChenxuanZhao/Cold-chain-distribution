...
代码还要在每个代码的前面加上0X，如原来是 FF 38 46 ...
要改为：0XFF,0X38,0X46...,粘贴到keil上组成数
组代替原来的数组就可以了。不过在代码中还要修改设置图片是设置的大小，这个大小在刚才打开图片的时候有显示
"at+cmmsdown=\"pic\",4051,100000\r"
中断4051是图片大小，根据不同图片大小而改变
*************************************************************/
...
void main() {
  ...
  while (1) {
    ...
    if (KEY5_PRESS)  //按键5发送指令拨打10086电话号码
    {
      ...
      DelaySec(1);
      Uart2Sends(
          "AT+CMMSEDIT=0\r");  //关闭编辑状态，这样在模块的buff区的数据将被删除
      DelaySec(1);
      Uart2Sends("AT+CMMSEDIT=1\r");  //打开编辑状态，这个状态才可以发送mms
      DelaySec(1);
      Uart2Sends(
          "at+cmmsdown=\"pic\",4051,100000\r");  //设置发送的图片的大小(byte)和需要用来传输的时间100000ms，4051这个数字从sscom32.exe得到的，打开图片的时候显示在最上面
      DelaySec(1);
      i = 0;
      while (
          i <
          sizeof(
              hua))  //发送图片的数据给模块，该数据用sscom32.exe得到，该软件在调试工具文件夹中
      {
        Uart2Send(hua[i]);
        i++;
      }
      ...
      DelaySec(2);
      Uart2Sends(
          "at+cmmsdown=\"title\",3,30000\r");  //设置发送的mms的名字，大小，时间
      DelaySec(1);
      Uart2Sends("hua");  //发送名字，hua
      DelaySec(1);
      Uart2Sends(
          "at+cmmsdown=\"text\",5,30000\r");  //设置发送的mms的文本内容，小于1000bytes
      DelaySec(1);
